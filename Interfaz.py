...
#########
        ...
        # Crear enemigo 2
        self.enemigo2 = enemigoscazador2(
                                        self.canvas_cazador,
                                        self.matriz_terrenos_cazador
                                        ) 
        
        # Crear enemigo 1
        self.enemigo3 = enemigoscazador3(
                                        self.canvas_cazador,
                                        self.matriz_terrenos_cazador
                                        ) 
        
        #se le asinga al enemigo #1 la posición real del cazador 
        ...
        #se le asinga al enemigo #2 la posición real del cazador 
        self.enemigo2.posicion_inicial_cazador_ejey = self.posicion_inicial_cazador_ejey
        self.enemigo2.posicion_inicial_cazador_ejex = self.posicion_inicial_cazador_ejex
        
        #se le asinga al enemigo #3 la posición real del cazador 
        self.enemigo3.posicion_inicial_cazador_ejey = self.posicion_inicial_cazador_ejey
        self.enemigo3.posicion_inicial_cazador_ejex = self.posicion_inicial_cazador_ejex
...
class enemigoscazador1: 
    ...
    def __init__(self,canvas_juego, mapa_terrenos):
            ...
            # Se incializa las variables del retroceso (evita que se encicle el enemigo al entrar al tunel sin salida)
            self.retroceso_usado = False
            self.estaba_en_tunel = False
...
###############################################
# Función que evita que el enemigo #1 aparezca de forma aleatoria en una celda que le impida moverse 

    def validar_posicion_inicial_enemigo(self, fila, columna):

        # Se Verifica que la celda en la que aparezca el enemigo #1 sea válida (no muro)
        if not self.es_posicion_valida(fila, columna):
            return False

        # Se evita que el enemigo aparecer donde está el cazador
        if (fila, columna) == (self.posicion_inicial_cazador_ejey,
                            self.posicion_inicial_cazador_ejex):
            return False

        # Se valida que el enemigo donde aparezca tenga al menos un movimiento posible al su alrededor 
        vecinos = [
                    (fila-1, columna),
                    (fila+1, columna),
                    (fila, columna-1),
        ...
        # Si la celda donde aparece el enemigo no es válida se recorre otra vez las opciones hasta encontrar una celda válida
        for f, c in vecinos:
...
class enemigoscazador2: 
    ...
    def actualizar_posicion2(self, fila_nueva, columna_nueva, direccion):
        ...
        # Se cambia la imagen del enemigo según su dirección
        if direccion == "arriba":
            self.imagen_actual2 = self.imagen_enemigo2_arriba
        elif direccion == "abajo":
            self.imagen_actual2 = self.imagen_enemigo2_abajo
        elif direccion == "izquierda":
            self.imagen_actual2 = self.imagen_enemigo2_izquierda
        elif direccion == "derecha":
            self.imagen_actual2 = self.imagen_enemigo2_derecha
...
###############################################        
    # Función que permite mover al enemigo #2 del juego del cazador de forma aleatoria 
    def mover_enemigo2(self):
        ...
        ## Si la celda sobre la que se esta es una salida se elimina al enemigo 
        if isinstance(celda_actual, Salida):
            print(f" El enemigo #2 ha encontrado una salida y sale de este en {fila_actual} {columna_actual}")
            self.canvas_juego.delete(self.enemigo2_objeto)
            return

        print(f"\n>>> ENEMIGO #2 EN: {fila_actual} {columna_actual}")
...
###############################################
    ...
    def actualizar_posicion3(self, fila_nueva, columna_nueva, direccion):
        ...
        # Se cambia la imagen del enemigo según su dirección
        if direccion == "arriba":
            self.imagen_actual3 = self.imagen_enemigo3_arriba
        elif direccion == "abajo":
            self.imagen_actual3 = self.imagen_enemigo3_abajo
        elif direccion == "izquierda":
            self.imagen_actual3 = self.imagen_enemigo3_izquierda
        elif direccion == "derecha":
            self.imagen_actual3 = self.imagen_enemigo3_derecha
